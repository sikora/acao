...
function generateFormFromSimpleTypeNodeRestrictionEnumeration(tagRaiz, xmlNode, namePattern, name, label, minOccurs){
    ...
    var classes = '';
    var restrictionNode = getNodeByTagName(xmlNode, 'xs:restriction');
    if(restrictionNode.getAttribute('autoComplete') == 1) 
        classes += ' autoComplete';


    if (minOccurs > 0) {
        classes += 'xsdForm__mandatory';
        newSelect.setAttribute('title', 'Este ítem é obrigatório!');
        newSelect.setAttribute('need', '1');
    }

    newSelect.setAttribute('class', classes );
...
function rodaAutoComplete() {
	    $(".autoComplete").searchable({
	        ...
	        warnNoMatch: 'não encontrado ...',      // string to show in the list when no entries match
	        zIndex: 'auto'                          // zIndex for elements generated by this plugin
	    });
}

//-----------------------------------   Dropbox com autocomplete   -------------------------------------------
 $(document).ready(function() {
    rodaAutoComplete();
	});
